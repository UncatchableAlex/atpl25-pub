...
import Numeric.LinearAlgebra
  ...
  , fromBlocks, subMatrix, cmap, cols, rows, scale, sumElements
  )
...
pauliOp op k r = go op
  where
    go = \case
      ...
      Permute ks -> error "pauliOp: Permute not implemented"
...
evalOpMat op k r = go op
  where
    go = \case
      ...
      H          -> (applyH1 k r)
      SX         -> (applySX1 k r)
      Permute ks -> error "evalOpMat: Permute not implemented"
